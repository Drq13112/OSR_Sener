...
class LaserScanProcessor(Node):
    def __init__(self):
        ...
        self.cmd_vel_pub = self.create_publisher(Twist, '/cmd_vel2',10)
    ...
    def publish_mid_marker(self):
        ...
        if self.centroid is not None and self.mean_mid is not None:
            ...
            t.transform.translation.x = abs(self.centroid[1])
            t.transform.translation.y = self.centroid[0]
    ...
    def pub_movebase(self):
        if self.new_mid_point is not None and self.quat is not None:
            ...
            msg.pose.position.x=1.5*abs(self.centroid[1])
            msg.pose.position.y=self.centroid[0]
